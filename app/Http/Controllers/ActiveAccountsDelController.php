...
class ActiveAccountsDelController extends Controller
{
    ...
    public function show_active_accounts_del($p_delegacion_id)
    {
        ...
        if ($p_delegacion_id <> $user_del_id) {
            Log::warning('Sin permiso-Consultar Lista Ctas Vigentes-Del distinta|' . $texto_log);
            return redirect('ctas')->with('message', 'No tiene permitido consultar el Listado de Ctas Vigentes de otra OOAD');
        }

        if ( !Auth::user()->hasRole('capturista_delegacional') || !Gate::allows( 'ver_lista_ctas_vigentes_del') ) {
            Log::warning('Sin permiso-Consultar Lista Ctas Vigentes-Del|' . $texto_log);
            return redirect('ctas')->with('message', 'No tiene permitido consultar el Listado de Ctas Vigentes de la OOAD');
        }
