    @if(count($solicitudes_con_respuesta_mainframe_ok))
        <h5 class="text-success">Total de solicitudes atendidas en Mainframe: {{ $solicitudes_con_respuesta_mainframe_ok->count() }}</h5>
    ...
    @forelse( $solicitudes_con_respuesta_mainframe_ok as $row_tabla_mov )
        @php
            ...
            <tr class="table-{{$color}}">
                ...
                <td class="small">
                    <a target="_blank" href="/ctas/solicitudes/{{ $row_tabla_mov->id }}">
                        {{ $row_tabla_mov->resultado_solicitud->cuenta }}
                    </a>
                </td>
    ...
    @empty
        <h5 class="text-warning">
            Aún no hay solicitudes con respuesta Mainframe
            @if( isset( $info_lote ) )
                Lote: {{ $info_lote->num_lote }}
            @else
                (Sin lote asignado)
